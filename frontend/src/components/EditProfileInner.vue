  ...
  <div class="w-full h-screen max-h-screen overflow-hidden">
    <div class="flex flex-col bg-gray-900 bg-opacity-20">
      ...
      >
        ...
        <div v-if="profile" class="flex items-center text-2xl">
...
const profile = ref<ReleaseProfile>(props.profile);
const uneditedProfile = ref<ReleaseProfile>(props.profile);
...
const run = () => {
  ...
  Run(computedCode.value, env, (ok: boolean, err: string | null, result: any) => {
    if (!ok) {
      console.error(err)
...
const setUneditedProfile = (name: string, filter: string, sorter: string) => {
  uneditedProfile.value.name = name;
  uneditedProfile.value.filter = filter;
  uneditedProfile.value.sorter = sorter;
};
...
const resetEditingName = () => {
  profile.value.name = oldName.value;
