  ...
  <Modal :title="title" v-model="computedActive" @close="emit('close')">
    <div>
      <template v-for="field in fields" :key="field.property">
        ...
        <o-field v-else-if="field.type === 'radio'" :label="field.label">
        ...
        <o-field class="my-2" v-else-if="field.type === 'switch'">
          <span class="my-1">
            <o-switch v-model="computedValue[field.property]"><span class="ml-1">{{ field.label }}</span></o-switch>
          </span>
        </o-field>
