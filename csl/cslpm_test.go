...
func TestParseGit1(t *testing.T) {
	importedScript, err := ParseImport("gitlab.com/lsnow/mycsl:main.csl")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseGit2(t *testing.T) {
	importedScript, err := ParseImport("github.com/lsnow99/mycsl:/main.csl@715ea4ca5c982ce83f60323329305554481fab34")
	if err != nil {
		t.Fatal(err)
	}
	gs, ok := importedScript.(GitScript)
...
func TestParseGit3(t *testing.T) {
	importedScript, err := ParseImport("github.com/lsnow99/mycsl:scripts/main.csl")
	if err != nil {
		t.Fatal(err)
	}
	gs, ok := importedScript.(GitScript)
...
func TestParseWeb(t *testing.T) {
	...
	importedScript, err := ParseImport(scriptURL)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseFileBasic(t *testing.T) {
	...
	importedScript, err := ParseImport("file:" + filePath)
	if err != nil {
		t.Fatal(err)
	}
